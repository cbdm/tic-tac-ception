...
def split_game(moves):
    ...
    for i, m in enumerate(moves):
        ...
        small_wins = [
            2
            if small_wins[3 * i + j] == new_game.get_turn()
            else (1 if small_wins[3 * i + j] is None else 0)
            for j in range(3)
            for i in range(3)
        ]
        ...
        for i in range(3):
            for j in range(3):
                ...
                cur_board = [
                    2
                    if cur_board[i][j] == new_game.get_turn()
                    else (1 if cur_board[i][j] is None else 0)
                    for j in range(3)
                    for i in range(3)
                ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i",
        "--file_in",
        help="filename to read the generated games",
        required=True,
    ...
    parser.add_argument(
        "-o",
        "--file_out",
        help="filename export the numpy array with parsed data",
        required=True,
